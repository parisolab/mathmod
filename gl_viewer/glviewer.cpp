...
void OpenGlWidget::CalculateTexturePoints(ModelType type)
{
    double tmp, val[6];
    uint Jprime;
    ...
    if(type == PAR_TYPE || type == PAR_4D_TYPE )
    {
    ...
    for(uint i =0; i < LocalScene.VertxNumber; i++)
    {
        ...
        if(type != 1)
        {
            ...
            val[3] = val[3] * ParObjet->masterthread->dif_u[0]  + ParObjet->masterthread->u_inf[0];
...
void OpenGlWidget::PutObjectInsideCube()
{
    ...
    for (uint i=0; i< LocalScene.VertxNumber; i++)
    {
        LocalScene.ArrayNorVer_localPt[10*i+7] = float(hauteur_fenetre*(double(LocalScene.ArrayNorVer_localPt[10*i+7]) + decalage_xo)/difMaximum);
...
static void DrawParIso (ObjectProperties *scene, int idx)
{
    ...
    if(scene->componentsinfos.ThereisRGBA[idx])
    ...
    if(scene->componentsinfos.ThereisCND[idx])
    {
        if(scene->componentsinfos.DFTrianglesVerifyCND[idx])
            glDrawElements(
                GL_TRIANGLES,
                int(3*(scene->componentsinfos.NbTrianglesVerifyCND[idx])),
                GL_UNSIGNED_INT,
                &(scene->PolyIndices_localPt[start_triangle])
            );

        if(scene->componentsinfos.DFTrianglesNotVerifyCND[idx])
            glDrawElements(
                GL_TRIANGLES,
                int(3*(scene->componentsinfos.NbTrianglesNotVerifyCND[idx])),
                GL_UNSIGNED_INT,
                &(scene->PolyIndices_localPt[3*scene->componentsinfos.NbTrianglesVerifyCND[idx]+start_triangle])
            );
    }



    for(uint i=0; i< scene->componentsinfos.NbComponents[idx]; i++)
    {
        if(!scene->componentsinfos.ThereisRGBA[idx])
        {
            for(uint j=0; j<4; j++)
        ...
        if(!scene->componentsinfos.ThereisCND[idx])
        {
            glDrawElements(
                GL_TRIANGLES,
                int(3*scene->componentsinfos.ParisoPositions[2*i+1]),
                GL_UNSIGNED_INT,
...
static void DrawIsoCND(ObjectProperties *scene, uint idx)
{
    int start_triangle= scene->componentsinfos.ParisoPositions[2*idx];
    if(scene->componentsinfos.DMTrianglesVerifyCND[idx])
    {
        glLineWidth(1);
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        glDrawElements(
    ...
    if(scene->componentsinfos.DMTrianglesNotVerifyCND[idx])
    {
        glLineWidth(1);
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        glDrawElements(
    ...
    if(scene->componentsinfos.DMTrianglesBorderCND[idx])
    {
        glLineWidth(4);
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        glDrawElements(
            GL_TRIANGLES,
            int(3*scene->componentsinfos.NbTrianglesBorderCND[idx]),
            GL_UNSIGNED_INT,
            &(scene->PolyIndices_localPt[3*(scene->componentsinfos.NbTrianglesVerifyCND[idx] + scene->componentsinfos.NbTrianglesNotVerifyCND[idx]) +
                start_triangle]));
        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
        glLineWidth(1);
    }
...
static void DrawMeshParIso(ObjectProperties *scene)
{
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
static void DrawMinimalTopology (ObjectProperties *scene)
{
    ...
    for (uint i = 0; i < scene->NbPolygnNbVertexPtMin; i++)
    {
        ...
        for (uint j = 0; j < polysize; j++)
        {
            ...
            glVertex3f(
                scene->ArrayNorVer_localPt[10*actualpointindice+7],
...
static void draw(ObjectProperties *scene)
{
    ...
    if (scene->fill == 1 /* && (scene->typedrawing == 1 ||  scene->componentsinfos.pariso)*/)
        for(uint i=0; i<scene->componentsinfos.parisosize; i++)
        DrawParIso(scene, i);

    // Draw Mesh Object:
    if (scene->triangles == 1)
        DrawMeshParIso(scene);
    ...
    for(uint i=0; i<scene->componentsinfos.parisosize; i++)
        if(scene->activarecnd && scene->componentsinfos.ThereisCND[i] /* && (scene->typedrawing == 1|| scene->componentsinfos.pariso)*/)
            DrawIsoCND(scene, i);
...
void OpenGlWidget::transparencypar(int cl, int currentposition)
{
    ...
    {
    case 1:
        LocalScene.frontcolsPar[(currentposition%10)*4+3] = (cl/255.0f) ;
        break;
    case 0:
        LocalScene.backcolsPar[(currentposition%10)*4+3] = (cl/255.0f);
        break;
    case 2:
        LocalScene.gridcol[3] = (cl/255.0f);
        break;
...
void OpenGlWidget::redpar(int cl, int currentposition)
{
    ...
    {
    case 1:
        LocalScene.frontcolsPar[(currentposition%10)*4+0] = (cl/255.0f);
        break;
    case 0:
        LocalScene.backcolsPar[(currentposition%10)*4+0] = (cl/255.0f);
        break;
    case 2:
        LocalScene.gridcol[0] =  (cl/255.0f);
        break;
...
void OpenGlWidget::greenpar(int cl, int currentposition)
{
    ...
    {
    case 1:
        LocalScene.frontcolsPar[(currentposition%10)*4+1] = (cl/255.0f);
        break;
    case 0:
        LocalScene.backcolsPar[(currentposition%10)*4+1] = (cl/255.0f);
        break;
    case 2:
        LocalScene.gridcol[1] = (cl/255.0f);
        break;
...
void OpenGlWidget::bluepar(int cl, int currentposition)
{
    ...
    {
    case 1:
        LocalScene.frontcolsPar[(currentposition%10)*4+2] = (cl/255.0f);
        break;
    case 0:
        LocalScene.backcolsPar[(currentposition%10)*4+2] = (cl/255.0f);
        break;
    case 2:
        LocalScene.gridcol[2] = (cl/255.0f);
        break;
